...
import com.arthurnagy.staysafe.feature.Type
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? =
        TypeChooserBinding.inflate(inflater, container, false).also {
            it.certificate.setOnClickListener {
                findNavController().navigate(TypeChooserBottomSheetDirections.actionTypeChooserBottomSheetToNewDocumentFragment(Type.CERTIFICATE))
            }
            it.statement.setOnClickListener {
                findNavController().navigate(TypeChooserBottomSheetDirections.actionTypeChooserBottomSheetToNewDocumentFragment(Type.STATEMENT))
            }
        }.root
}